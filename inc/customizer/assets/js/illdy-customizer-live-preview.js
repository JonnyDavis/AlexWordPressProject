...
( function( $ ) {
	...
	// Show this section
	wp.customize( 'illdy_products_general_show', function( value ) {
		value.bind( function( newval ) {
			if( newval == false ) {
				$( '#products' ).addClass( 'customizer-display-none' );
			} else if( newval == true ) {
				$( '#products' ).removeClass( 'customizer-display-none' );
			}
		} );
	} );

	// Title
	wp.customize( 'illdy_products_general_title', function( value ) {
		value.bind( function( newval ) {
			$( '#products.front-page-section .section-header h3' ).html( newval );
		} );
	} );

	// Entry
	wp.customize( 'illdy_products_general_entry', function( value ) {
		value.bind( function( newval ) {
			$( '#products.front-page-section .section-header p' ).html( newval );
		} );
	} );
	// Show this section
